...
export default {
  ...
  computed: {
    ...
    canEdit() {
      return (
        this.hasPermission("modificar_roles") && this.rol.id != this.me.rol.id
      );
    },
  },
  watch: {
  ...
  methods: {
    ...
    toggle(permiso) {
      if (permiso.selected) {
        rolService.agregarPermiso(this.rol.id, permiso).then(() => {
          Alert.success("El permiso se agregó exitosamente.");
        });
      } else {
        rolService.eliminarPermiso(this.rol.id, permiso).then(() => {
